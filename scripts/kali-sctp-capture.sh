...
scp -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/tmp/pz-known-hosts -o ConnectTimeout=8 -i "$KEYDIR/id_ed25519" "$ROOT/scripts/guest/sctp.sh" root@$HOST:/tmp/protocol-zoo-sctp.sh
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/tmp/pz-known-hosts -o ConnectTimeout=8 -i "$KEYDIR/id_ed25519" root@$HOST sh /tmp/protocol-zoo-sctp.sh
