...
A=pz-m15-a; B=pz-m15-b; VA=pz-m15-va; VB=pz-m15-vb
TMP=; PIDS=""
...
mkdir -p "$OUT_DIR"
rm -f "$OUT_DIR/sit-ipv6-in-ipv4.pcapng" "$OUT_DIR/sit-ipv6-in-ipv4.frames.tsv" "$OUT_DIR/sit-ipv6-in-ipv4.json"
cleanup; TMP=$(mktemp -d /tmp/pz-m15.XXXXXX)
