...
OUT=${1:-captures/real-app-netns}; mkdir -p "$ROOT/$OUT"
...
editcap -F pcapng /tmp/pz-telnet-real.pcap "$ROOT/$OUT/telnet.pcapng"
...
sleep 1
printf 'user zoo zoo-pass\npassive\npassive\nlist\nget readme.txt /tmp/pz-ftp-retrieved\nquit\n' | sudo ip netns exec pz-client ftp -inv 198.18.0.2 2121 >"$ROOT/$OUT/ftp-passive.log" 2>&1
printf 'user zoo zoo-pass\nlist\nquit\n' | sudo ip netns exec pz-client ftp -inv 198.18.0.2 2121 >"$ROOT/$OUT/ftp-active.log" 2>&1
sleep 1
for p in $PIDS; do sudo kill -INT "$p" 2>/dev/null || true; done; PIDS=""
editcap -F pcapng /tmp/pz-ftp-real.pcap "$ROOT/$OUT/ftp.pcapng"
tshark -r "$ROOT/$OUT/telnet.pcapng" -T fields -E header=y -e frame.number -e ip.src -e ip.dst -e tcp.srcport -e tcp.dstport -e tcp.payload >"$ROOT/$OUT/telnet.frames.tsv" 2>/dev/null || true
tshark -r "$ROOT/$OUT/ftp.pcapng" -d tcp.port==2121,ftp -d tcp.port==19009,ftp-data -T fields -E header=y -e frame.number -e ip.src -e ip.dst -e tcp.srcport -e tcp.dstport -e ftp.request.command -e ftp.response.code >"$ROOT/$OUT/ftp.frames.tsv" 2>/dev/null || true
