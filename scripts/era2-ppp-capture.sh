...
pz_require_capture_children "$ROOT" "$OUT" \
  ...
  ppp-icmp.frames.tsv \
  ppp.json
A=pz-m11-a; B=pz-m11-b; VA=pz-m11-va; VB=pz-m11-vb
TMP=; PIDS=""; CAP_PID=""; OK=0
...
mkdir -p "$OUT_DIR"
rm -f \
  "$OUT_DIR/serial-hdlc.txt" \
  "$OUT_DIR/pppd-a.log" \
  "$OUT_DIR/pppd-b.log" \
  "$OUT_DIR/ppp-icmp.pcapng" \
  "$OUT_DIR/ppp-icmp.frames.tsv" \
  "$OUT_DIR/ppp.json"
cleanup; TMP=$(mktemp -d /tmp/pz-m11.XXXXXX); sudo chmod 777 "$TMP"
...
socat -d -d -x -v PTY,link="$TMP/pty-a",raw,echo=0 PTY,link="$TMP/pty-b",raw,echo=0 >"$OUT_DIR/serial-hdlc.txt" 2>&1 & PIDS="$PIDS $!"
sleep 1
...
sudo ip netns exec "$A" /usr/sbin/pppd "$TMP/pty-a" 198.18.170.1:198.18.170.2 noauth nodetach local lock asyncmap 0 mtu 1500 mru 1500 >"$OUT_DIR/pppd-a.log" 2>&1 & PIDS="$PIDS $!"
sudo ip netns exec "$B" /usr/sbin/pppd "$TMP/pty-b" 198.18.170.2:198.18.170.1 noauth nodetach local lock asyncmap 0 mtu 1500 mru 1500 >"$OUT_DIR/pppd-b.log" 2>&1 & PIDS="$PIDS $!"
sleep 5
...
sleep 1
FRAMES=0
if [ -s "$TMP/ppp-a.pcap" ]; then sudo editcap -F pcapng "$TMP/ppp-a.pcap" "$OUT_DIR/ppp-icmp.pcapng"; sudo chown "$(id -u):$(id -g)" "$OUT_DIR/ppp-icmp.pcapng"; FRAMES=$(tshark -r "$OUT_DIR/ppp-icmp.pcapng" -T fields -e frame.number 2>/dev/null | wc -l); fi
# Require all three evidence legs before calling this real-capture.
if grep -Eq 'local +IP address' "$OUT_DIR/pppd-a.log" 2>/dev/null && grep -Eq 'local +IP address' "$OUT_DIR/pppd-b.log" 2>/dev/null && [ "$FRAMES" -gt 0 ] && grep -Eqi 'c0 21' "$OUT_DIR/serial-hdlc.txt" && grep -Eqi '80 21' "$OUT_DIR/serial-hdlc.txt" && grep -Eqi '7e 21' "$OUT_DIR/serial-hdlc.txt"; then OK=1; fi
if [ "$OK" -eq 1 ]; then
  tshark -r "$OUT_DIR/ppp-icmp.pcapng" -T fields -E header=y -e frame.number -e ip.src -e ip.dst -e icmp.type -e icmp.code >"$OUT_DIR/ppp-icmp.frames.tsv"
  cat >"$OUT_DIR/ppp.json" <<EOF
...
jsonschema -i "$OUT_DIR/ppp.json" "$ROOT/schemas/experiment.schema.json"
