...
PIDS=""; TMP=$(mktemp -d /tmp/pz-era2-network.XXXXXX)
...
mkdir -p "$ROOT/$OUT"; rm -f "$ROOT/$OUT"/*
...
sudo editcap -F pcapng "$TMP/icmp.pcap" "$ROOT/$OUT/icmp-lifecycle.pcapng"; sudo chown "$(id -u):$(id -g)" "$ROOT/$OUT/icmp-lifecycle.pcapng"
...
sleep 1
for p in $PIDS; do sudo kill -INT "$p" 2>/dev/null || true; done; for p in $PIDS; do wait "$p" 2>/dev/null || true; done; PIDS=""
sudo editcap -F pcapng "$TMP/igmp.pcap" "$ROOT/$OUT/igmp-membership.pcapng"; sudo chown "$(id -u):$(id -g)" "$ROOT/$OUT/igmp-membership.pcapng"

for name in icmp-lifecycle igmp-membership; do
  tshark -r "$ROOT/$OUT/$name.pcapng" -T fields -E header=y -e frame.number -e ip.src -e ip.dst -e ip.proto -e icmp.type -e icmp.code -e igmp.type -e igmp.maddr >"$ROOT/$OUT/$name.frames.tsv" 2>/dev/null
  frames=$(tshark -r "$ROOT/$OUT/$name.pcapng" -T fields -e frame.number 2>/dev/null | wc -l)
  [ "$frames" -gt 0 ]
  proto=icmp; [ "$name" = igmp-membership ] && proto=igmp
  cat >"$ROOT/$OUT/$name.json" <<EOF
...
EOF
  jsonschema -i "$ROOT/$OUT/$name.json" "$ROOT/schemas/experiment.schema.json"
done
# Require the mechanisms we claim.
tshark -r "$ROOT/$OUT/icmp-lifecycle.pcapng" -Y 'icmp.type == 0' -T fields -e frame.number | grep -q .
tshark -r "$ROOT/$OUT/icmp-lifecycle.pcapng" -Y 'icmp.type == 3 && icmp.code == 3' -T fields -e frame.number | grep -q .
tshark -r "$ROOT/$OUT/igmp-membership.pcapng" -Y 'igmp' -T fields -e frame.number | grep -q .
