...
实验基础设施和第一批研究展品已完成；Telnet、FTP 及第二纪元部分传输/隧道展品已有成熟实现或内核的 namespace 隔离实测。第二纪元 M10–M19 的生态路线、数据集、研究矩阵和安全边界已建立；尚未成功的实测明确保留为 `not-run`。仓库优先记录可复现、可审查的实验；没有在当前环境真实跑通的协议，会明确标为 `fixture`、`static` 或 `not-run`，不把推导冒充实测。
